...
We can see this reflected in the table below, which shows data from the current ladder and a predicted end-of-season ladder based on Sqwiggle created prior to round 1. While both ladders have North Melbourne sitting on top undefeated, other teams shift position.

\- Carlton finished `r combined_ladder |> filter(str_detect(team_club_name,"Carlton")) |>  pull(position_current) |>  toOrdinal::toOrdinal()` but were only projected to finish `r combined_ladder |> filter(str_detect(team_club_name,"GWS")) |>  pull(position_predicted) |>  toOrdinal::toOrdinal()`.

\- Hawthorn, who finished `r combined_ladder |> filter(str_detect(team_club_name,"Hawthorn")) |>  pull(position_current) |>  toOrdinal::toOrdinal()` were predicted to finish`r combined_ladder |> filter(str_detect(team_club_name,"Hawthorn")) |>  pull(position_predicted) |>  toOrdinal::toOrdinal()` but benefitted from upsetting Brisbane in the opening round `r combined_ladder |> filter(str_detect(team_club_name,"Hawthorn")) |>  pull(opposition_strength)`).

\- Port Adelaide were predicted to finish `r combined_ladder |> filter(str_detect(team_club_name,"Port")) |>  pull(position_predicted) |>  toOrdinal::toOrdinal()` even though Sqwiggle ranks them as the `r combined_ladder |> filter(str_detect(team_club_name,"Port")) |>  pull(position_model) |>  toOrdinal::toOrdinal()` best team as they have the toughest draw (opposition strength of `r combined_ladder |> filter(str_detect(team_club_name,"Port")) |>  pull(opposition_strength)`).
